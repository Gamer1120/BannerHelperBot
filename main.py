...
async def first_mission(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'https://link.ingress.com/?link=' not in shared_url:
        ...
        if resp.status_code is not 200:
            await context.bot.send_message(chat_id=update.effective_chat.id, text='Bannergress did not like that mission. Please report the mission to @M1chaeI so he can investigate.')
        else:
            j = resp.json()
            mission_details = j[0]
            reply_keyboard = [
...
async def next_mission(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if resp.status_code is not 200:
        ...
        if int(mission_number) > int(total_number_of_missions_in_banner):
            await query.edit_message_text(text='Looks like you completed the banner. Congrats!')
        else:
